...
| Qwen3.5-0.5B @ 128K 컨텍스트 | 10.50 GB | 2.79 GB | **7.71 GB 절약 (73%)** |
...
**실제** [Qwen3.5-0.8B](https://huggingface.co/Qwen/Qwen3.5-0.8B) KV 캐시 (합성 아닌 실제 모델 추론 데이터)로 검증:
...
**실제 모델 데이터 핵심 발견:**
- `uniform_4b`와 `mixed_4b8` 모두 실제 Qwen3.5 KV 캐시에서 **A+ (코사인 0.994)**
- `uniform_2b`도 **A등급 (0.953)** — 14배 압축에서도 고품질
- **RHT가 MSE를 1.8배 감소** (0.038 → 0.021)
...
- **실제 모델 검증** — Qwen3.5-0.5B KV 캐시 패턴, 코사인 0.991
